...
export const createBranchInfo = yup.object().shape({
  numberOfEmployees: yup
    .number()
    .integer()
    .required(errorMessage("Number of employees"))
    .default(0),
  numberOfBranches: yup
    .number()
    .integer()
    .positive()
    .required(errorMessage("Number of branches")),
});
export const createBVNDetails = yup.object().shape({
  bvn: yup
    .string()
    .trim()
    .required(errorMessage("BVN"))
    .length(11)
    .label("BVN"),
  agreedToTerms: yup
    .boolean()
...
export const sendEmailSchema = yup.object().shape({
  toEmails: yup.string().required(errorMessage("Receiver(s) email(s)")),
...
export const editMerchantDetailsSchema = yup.object().shape({
  fullName: yup.string().required(errorMessage("Full name")),
  email: yup.string().required(errorMessage("Email")),
  phoneNumber: yup.string().required(errorMessage("Phone number")),
});
